...
function countRun(board: Cell[][], from: Coord, d: Coord, s: Stone): number {
  let n = 0
  for (let k = 1; k <= 4; k++) {
...
function isOpenEnd(board: Cell[][], from: Coord, d: Coord, len: number): boolean {
  const p = { r: from.r + d.r * (len + 1), c: from.c + d.c * (len + 1) }
  return inside(board, p) && cell(board, p) === null
}

function scoreCell(board: Cell[][], at: Coord, s: Stone): number {
  // 간단 휴리스틱:
...
export function pickAiMove(board: Cell[][], aiStone: Stone): Coord | null {
